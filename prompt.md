...
*   JSONの最も外側は、配列 `[]` とします。
*   画像の上部にある**主要なタイトル（例：「10-(3)...」、「10-(4)...」）を動的に抽出し**、それを省略せずに各オブジェクトの`indicator`キーの値として使用してください。複数の主要タイトルがある場合は、**必ずそれぞれを別のオブジェクトとして配列に含めてください。**
*   画像の下部にある**ページ番号**を抽出し、各オブジェクトの`page`キーの値として数値型で含めてください。
*   各オブジェクトは、`indicator`、`page`、`総計`、`データ一覧`というキーを持ちます。

**2. 列ヘッダーの動的抽出:**

*   `総計`オブジェクトおよび`データ一覧`内の各`data`オブジェクトで使用するキー（例: `取付棟数`, `箇所数`, `取付率`）は、**各主要タイトルの真下にある列ヘッダーを画像から直接読み取って使用してください。添付したHTMLにも情報があります。【重要】列ヘッダーのテキストは余計な文言を追加せず、書いてあるとおりのものを使用してください。** これにより、将来的に異なる列を持つ表にも対応できます。
...
*   **（参考）** 以下は、今回添付された画像から期待される出力形式の**一例**です。`indicator`の値（「10-(3)...」など）や列ヘッダー（「取付棟数」など）は、このルールに従って画像から**動的に抽出されたもの**であるべきです。
