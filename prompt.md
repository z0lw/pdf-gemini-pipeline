...
*   JSONの最も外側は、配列 `[]` とします。
*   画像の上部にある**主要なタイトル（例：「10-(3)...」、「10-(4)...」）を動的に抽出し**、それを省略せずに各オブジェクトの`indicator`キーの値として使用してください。複数の主要タイトルがある場合は、それぞれを別のオブジェクトとして配列に含めてください。
*   各オブジェクトは、`indicator`、`総計`、`データ一覧`という3つのキーを持ちます。
